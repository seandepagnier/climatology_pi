...
Segment::Segment(int I, int J,
                char c1, char c2, char c3, char c4,
                 ClimatologyOverlayFactory &fac, int setting,
                 double pressure)
{
    ...
    intersectionAreteGrille(i,j, k,l,  &px1,&py1, fac, setting, pressure);
    intersectionAreteGrille(m,n, o,p,  &px2,&py2, fac, setting, pressure);
}
//-----------------------------------------------------------------------
...
//            y = rec->getY(j);
            ...
            if     ((a<=value && b<=value && c<=value  && d>value)
                ...
                trace.push_back(new Segment(i,j, 'c','d',  'b','d', fac, setting, value));
            else if ((a<=value && c<=value && d<=value  && b>value)
                 || (a>value && c>value && d>value  && b<=value))
                trace.push_back(new Segment(i,j, 'a','b',  'b','d', fac, setting, value));
            else if ((c<=value && d<=value && b<=value  && a>value)
                 || (c>value && d>value && b>value  && a<=value))
                trace.push_back(new Segment(i,j, 'a','b',  'a','c', fac, setting, value));
            else if ((a<=value && b<=value && d<=value  && c>value)
                 || (a>value && b>value && d>value  && c<=value))
                trace.push_back(new Segment(i,j, 'a','c',  'c','d', fac, setting,value));
            //--------------------------------
            // 1 segment H ou V
            //--------------------------------
            else if ((a<=value && b<=value   &&  c>value && d>value)
                 || (a>value && b>value   &&  c<=value && d<=value))
                trace.push_back(new Segment(i,j, 'a','c',  'b','d', fac, setting,value));
            else if ((a<=value && c<=value   &&  b>value && d>value)
                 || (a>value && c>value   &&  b<=value && d<=value))
                trace.push_back(new Segment(i,j, 'a','b',  'c','d', fac, setting,value));
            //--------------------------------
            ...
            //--------------------------------
                ...
                trace.push_back(new Segment(i,j, 'a','b',  'a','c', fac, setting,value));
                trace.push_back(new Segment(i,j, 'b','d',  'c','d', fac, setting,value));
            }
